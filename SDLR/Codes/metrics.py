...
def ndcg(path,name,d,y_pred, y_true, ats=None, gain_function=lambda x: torch.pow(2, x) - 1, padding_indicator=PADDED_Y_VALUE,
         filler_value=1.0):
    """
    ...
    if d == 1:
        x_df = pd.DataFrame(ndcg_.cpu().detach().numpy())

        x_df.to_csv(name +path +'NDCG.csv', mode='a', header=False)
    return ndcg_
...
def mrr(path,name,d,y_pred, y_true, ats=None, padding_indicator=PADDED_Y_VALUE):
    """
    ...
    if d == 1:
        ...
        x_df=pd.DataFrame(result.cpu().detach().numpy())

        x_df.to_csv(name + path + 'MRR.csv', mode='a', header=False)
