...
class LibSVMDataset(Dataset):
    ...
    def __init__(self, X, y, query_ids, transform=None):
        ...
        for i in range(len(Unique_Labels)):
            ...
            Selected_Indices = np.where(y == Unique_Labels[i])[0]
            Temp = X[list(Selected_Indices), :]
            Temp = np.absolute(Temp)
            Temp_Mean = np.mean(Temp, axis=0)
            Mean_Label_Based += [Temp_Mean.tolist()]
            Temp_Sigma = (np.max(Temp, axis = 0) - np.min(Temp, axis = 0)) / 6.0
            Temp_Sigma[np.where(Temp_Sigma == 0)[0].tolist()] = 1
            Sigma_Label_Based += [Temp_Sigma.tolist()]
            Coefficient = 1  # torch.pow((1 / ( 2 * torch.pi * Temp_Sigma)), (len(Selected_Indices) / 2) )
            Temp[np.where(Temp == 0)] = float("-inf")
            Temp = np.multiply(Coefficient, np.exp(-1 * (np.power(np.subtract(Temp, Temp_Mean), 2) / (2 * Temp_Sigma))))

            Temp = np.log(np.linalg.norm(Temp, axis = 1) + 1e-10)


        Mean_Label_Based = pd.DataFrame(Mean_Label_Based, index = Unique_Labels)
        Mean_Label_Based.to_csv("./Parameters/0_Means_All.csv")
        Sigma_Label_Based = pd.DataFrame(Sigma_Label_Based, index = Unique_Labels)
        Sigma_Label_Based.to_csv("./Parameters/0_Sigmas_All.csv")
...
def create_data_loaders(train_ds: LibSVMDataset, val_ds: LibSVMDataset, num_workers: int, batch_size: int):
    ...
    for i in range(len(train_ds.X_by_qid)):
        ...
        ###px = torch.multiply(1 / (sigma + 1e-10), torch.exp(-1 * (torch.pow(px, 2) / (2 * sigma))))
        px = torch.exp( -1 * (torch.pow(px, 2) / (2 * sigma)) )
        #sigma = torch.load('sigma.pt')#.to(torch.device("cuda:0"))
        #torch.save(px, 'sigma.pt')
        px_x = torch.multiply(px, torch.from_numpy(train_ds.X_by_qid[i]))
        import pandas as pd
        Info = pd.DataFrame(np.array(px.data))
        Info.to_csv("PX.csv")
    ...
    print()
    ###XX = np.array(train_dl.dataset.X_by_qid)
